...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET': {
      ...
      return withApiHandler(res.status(200).json({ role, permissions }));
    }
    case 'PUT': {
      // 更新角色權限
      ...
      return res.status(200).json({ message: '角色權限已更新', role, permissions: newPermissions });
    }
    case 'POST': {
      // 重置角色權限為預設值
      ...
      return res.status(200).json({ message: '角色權限已重置為預設值', role, permissions: resetPermissions });
    }
    default:
      return res.status(405).json({ message: '方法不允許' });
  }
