...
BEGIN
  -- 獲取ADMIN角色ID
  SELECT id::TEXT INTO admin_role_id FROM "Role" WHERE name = 'ADMIN' LIMIT 1;
  
  IF admin_role_id IS NOT NULL THEN
    -- 檢查該角色是否已有權限
    SELECT COUNT(*) INTO perm_count FROM "RolePermission" WHERE "roleId" = admin_role_id;
    
    IF perm_count = 0 THEN
      -- 為ADMIN角色分配所有權限
      FOR perm_id IN SELECT id::TEXT FROM "Permission" LOOP
...
BEGIN
  -- 獲取MANAGER角色ID
  SELECT id::TEXT INTO manager_role_id FROM "Role" WHERE name = 'MANAGER' LIMIT 1;
  
  IF manager_role_id IS NOT NULL THEN
    -- 檢查該角色是否已有權限
    SELECT COUNT(*) INTO perm_count FROM "RolePermission" WHERE "roleId" = manager_role_id;
    
    IF perm_count = 0 THEN
      -- 為MANAGER角色分配權限
      FOREACH perm_name IN ARRAY perm_names LOOP
        SELECT id::TEXT INTO perm_id FROM "Permission" WHERE name = perm_name LIMIT 1;
...
BEGIN
  -- 獲取USER角色ID
  SELECT id::TEXT INTO user_role_id FROM "Role" WHERE name = 'USER' LIMIT 1;
  
  IF user_role_id IS NOT NULL THEN
    -- 檢查該角色是否已有權限
    SELECT COUNT(*) INTO perm_count FROM "RolePermission" WHERE "roleId" = user_role_id;
...
BEGIN
  FOR user_record IN SELECT id, role FROM "User" LOOP
    -- 檢查用戶是否已有角色分配
    SELECT COUNT(*) INTO user_role_count FROM "UserRole" WHERE "userId" = user_record.id;
    
    IF user_role_count = 0 THEN
      -- 獲取對應角色ID
      SELECT id::TEXT INTO role_id FROM "Role" WHERE name = user_record.role LIMIT 1;
...
BEGIN
  -- 獲取STAFF角色ID
  SELECT id::TEXT INTO staff_role_id FROM "Role" WHERE name = 'STAFF' LIMIT 1;
  
  IF staff_role_id IS NOT NULL THEN
    -- 檢查該角色是否已有權限
    SELECT COUNT(*) INTO perm_count FROM "RolePermission" WHERE "roleId" = staff_role_id;
...
BEGIN
  -- 獲取角色ID
  SELECT id::TEXT INTO role_id FROM "Role" WHERE name = 'REPORT_PROCESSOR' LIMIT 1;
  
  IF role_id IS NOT NULL THEN
    -- 檢查該角色是否已有權限
    SELECT COUNT(*) INTO perm_count FROM "RolePermission" WHERE "roleId" = role_id;
...
BEGIN
  -- 獲取角色ID
  SELECT id::TEXT INTO role_id FROM "Role" WHERE name = 'REPORT_REVIEWER' LIMIT 1;
...
BEGIN
  -- 獲取角色ID
  SELECT id::TEXT INTO role_id FROM "Role" WHERE name = 'CUSTOMER_SERVICE' LIMIT 1;
...
BEGIN
  -- 獲取角色ID
  SELECT id::TEXT INTO role_id FROM "Role" WHERE name = 'MAINTENANCE_WORKER' LIMIT 1;
