...
const CategorySelector: React.FC<CategorySelectorProps> = ({ onCategorySelect, selectedCategoryId }) => {
  ...
  if (error) {
    return <div className="text-center py-4 text-red-500">載入分類失敗: {error}</div>;
  }
        ...
        {categories.length === 0 ? (
          ...
          categories.map((category1) => (
            <div key={category1.id} className="border border-gray-200 rounded-md overflow-hidden mb-2">
              ...
              >
                <div className="font-medium text-gray-800">{category1.name}</div>
              ...
              {expandedLevel1.includes(category1.id) && (
                <div className="pl-4">
                  {category1.children && category1.children.map((category2) => (
                    <div key={category2.id} className="border-t border-gray-100">
                      {/* 二級分類標題 */}
                      <div className="px-4 py-2 bg-gray-50">
                        <div className="font-medium text-gray-700">{category2.name}</div>
                      </div>

                      {/* 三級分類選項，直接顯示 */}
                      <div className="pl-4 py-2 grid grid-cols-1 gap-2">
                        {category2.children && category2.children.map((category3) => (
