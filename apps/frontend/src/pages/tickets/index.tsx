          ...
          if (value.length > 0) {
            if (key === 'dateRange') {
              ...
              const normalizedRange = [
                start ? format(new Date(start), 'yyyy-MM-dd') : null,
                end ? format(new Date(end), 'yyyy-MM-dd') : null,
              ];
  ...
  return (
    <>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {tickets.map((ticket) => (
                    ...
                    >
                      ...
                      <td className="p-0">
                        <TooltipCell
                          content={ticket.role?.description || '未指派'}
                          className="px-2 py-3 whitespace-nowrap text-sm text-gray-500 md:text-ellipsis md:overflow-hidden w-full h-full"
                        >
                          {ticket.role?.description || '未指派'}
                        </TooltipCell>
                      </td>
                      <td className="p-0">
                        <TooltipCell
            ...
            <div className="flex justify-between items-center mt-4 p-4">
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">
                  {totalTickets > 0 ? (page - 1) * 20 + 1 : 0}
                </span>{' '}
                到{' '}
                <span className="font-medium">
                  {Math.min(page * 20, totalTickets)}
                </span>{' '}
      ...
      <MultiSelectFilterModal
        ...
        onConfirm={(selected) => {
          updateQuery({ roleIds: selected });
          setIsRoleModalOpen(false);
        }}
        initialSelectedIds={roleIds}
        options={allRoles.map((r) => ({ id: r.id, name: r.description }))}
        title="選擇指派角色"
      />
      <MultiSelectFilterModal
        ...
        options={allUsers.map((c) => ({ id: c.id, name: c.name }))}
        title="選擇建立者"
      />
      <MultiSelectFilterModal
        ...
        onConfirm={(selected) => {
          updateQuery({ assigneeIds: selected });
          setIsAssigneeModalOpen(false);
        }}
        initialSelectedIds={assigneeIds}
        options={allAssignees.map((a) => ({ id: a.id, name: a.name }))}
        title="選擇負責人"
      />
    </>
  );
