...
export default function TicketsPage() {
  ...
  return (
    <>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {tickets.map((ticket) => (
                    <tr key={ticket.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => router.push(`/tickets/${ticket.id}?returnUrl=${encodeURIComponent(router.asPath)}`)}>
                      ...
                      <td className="p-0"><TooltipCell content={ticket.role?.description || '未指派'} className="px-2 py-3 whitespace-nowrap text-sm text-gray-500 md:text-ellipsis md:overflow-hidden w-full h-full">{ticket.role?.description || '未指派'}</TooltipCell></td>
      ...
      <MultiSelectFilterModal isOpen={isRoleModalOpen} onClose={() => setIsRoleModalOpen(false)} onConfirm={(selected) => { updateQuery({ roleIds: selected }); setIsRoleModalOpen(false); }} initialSelectedIds={roleIds} options={allRoles.map((r) => ({ id: r.id, name: r.description }))} title="選擇指派角色" />
