...
const toast = {
  ...
  error: (message: string) => { alert(`錯誤: ${message}`); }
};
...
const RolesManagementPage: React.FC = () => {
  ...
  const [currentUser, setCurrentUser] = useState<{ role: UserRole }>({ role: UserRole.ADMIN }); // 模擬當前用戶
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      setIsSaving(true);
      const result = await roleService.updateRolePermissions(selectedRole, rolePermissions);
      ...
      console.error(`Error saving permissions for role ${selectedRole}:`, error);
      toast.error('保存權限失敗');
    } finally {
      setIsSaving(false);
    }
  ...
  const handleResetPermissions = async () => {
    ...
    try {
      ...
      console.error(`Error resetting permissions for role ${selectedRole}:`, error);
      toast.error('重置權限失敗');
    } finally {
  ...
  const permissionGroups = {
    tickets: allPermissions.filter(p => p.startsWith('view_tickets') || p.startsWith('create_tickets') || 
                                     p.startsWith('edit_tickets') || p.startsWith('delete_tickets') || 
                                     p.startsWith('assign_tickets') || p.startsWith('claim_tickets') || 
                                     p.startsWith('complete_tickets') || p.startsWith('verify_tickets')),
    reports: allPermissions.filter(p => p.startsWith('view_reports') || p.startsWith('create_reports') || 
                                     p.startsWith('process_reports') || p.startsWith('review_reports')),
    users: allPermissions.filter(p => p.startsWith('view_users') || p.startsWith('create_users') || 
                                   p.startsWith('edit_users') || p.startsWith('delete_users')),
    roles: allPermissions.filter(p => p.startsWith('manage_roles') || p.startsWith('assign_permissions'))
  };
            ...
            <div className="bg-white p-4 rounded shadow">
              ...
              <div className="space-y-2">
                {Object.values(UserRole).map(role => (
                  <div 
                    key={role}
                    className={`p-2 rounded cursor-pointer ${selectedRole === role ? 'bg-blue-100 font-medium' : 'hover:bg-gray-100'}`}
